...
def _eye_field(
    ...
    *,
    ...
    radius_km = torch.sqrt(
        (east_km - center_east_km).square() + north_km.square()
    )
    eye = 15.0 * torch.exp(-0.5 * (radius_km / 10.0).square())
    eyewall = 25.0 * torch.exp(
        -0.5 * ((radius_km - 45.0) / 10.0).square()
    )
...
def test_eye_center_displacement_tracks_shifted_low_wind_eye() -> None:
    ...
    prediction = _eye_field(
        east_km, north_km, center_east_km=30.0
    )[None, None]
...
def test_eye_center_displacement_skips_outer_target_minimum() -> None:
    ...
    outer_low = 18.0 * torch.exp(
        -0.5
        * (((east_km - 80.0) / 8.0).square() + (north_km / 8.0).square())
    )
