...
def _geo_frame(bundle: dict, label: str) -> np.ndarray:
    ...
    value = np.clip((field - GEOSTAT_SCALE_MIN_K) / (GEOSTAT_SCALE_MAX_K - GEOSTAT_SCALE_MIN_K), 0, 1)
    ...
    rgb[valid] = (np.array([255, 255, 255]) + value[valid, None] * (np.array([22, 82, 180]) - np.array([255, 255, 255]))).astype(np.uint8)
    return np.asarray(_caption(Image.fromarray(rgb).resize((SIZE, SIZE), Image.Resampling.LANCZOS), label))


def _prediction_frame(field: np.ndarray, valid: np.ndarray, label: str, vmin: float, vmax: float) -> np.ndarray:
...
def main() -> None:
    ...
    records = _read_manifest(DEFAULT_DATA_ROOT / "index-files" / "observation_manifest_v6.csv", DEFAULT_DATA_ROOT)
    by_id = {record.observation_id: record for record in records}
    era5 = {}
    for storm in STORMS:
        record = next(item for item in records if item.storm_id == storm and item.source_type == "era5")
    ...
    model = ERA5ResidualRegressor.load_from_checkpoint(DEFAULT_CHECKPOINT, map_location="cpu").eval().cuda()
    geo_frames, fields, masks, labels = [], [], [], []
    with torch.inference_mode():
        for storm in STORMS:
            table = pd.read_csv(DEFAULT_REFERENCE_ROOT / storm / "inference-summary.csv").sort_values("observation_timestamp")
            for row in table.itertuples(index=False):
                bundle = torch.load(DEFAULT_REFERENCE_ROOT / storm / row.inference_path, map_location="cpu", weights_only=False)
                labels.append(f"{storm}  {pd.Timestamp(row.observation_timestamp):%Y-%m-%d %H:%M UTC}")
                geo_frames.append(_geo_frame(bundle, labels[-1]))
                batch, _ = _prepare_sample(by_id[row.observation_id], era5[storm], stats)
                valid = batch["condition_mask"].squeeze().numpy().astype(bool)
                prediction = model.predict_physical({key: value.cuda() for key, value in batch.items()}).squeeze().cpu().numpy()
    ...
    for index, field in enumerate(fields):
        ...
        weights = np.array([1.0 if item == index else 0.5 for item in range(start, end)], dtype=np.float32)
