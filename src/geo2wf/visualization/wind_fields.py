...
def _resolve_pair_path(
    ...
    if not _has_value(row, column):
        raise KeyError(
            f"Manifest row is missing {primary_column!r}/{fallback_column!r}"
        )
...
def _read_raster_view(path: Path, band: int) -> dict[str, Any]:
    with rasterio.open(path) as dataset:
        if band < 1 or band > dataset.count:
            raise ValueError(
                f"{path} has {dataset.count} bands, cannot read band {band}"
            )
...
def _read_geo_false_color_view(
    ...
    with rasterio.open(path) as dataset:
        ...
        mask &= ~np.logical_and.reduce(
            [np.isclose(channel, 0.0) for channel in channels]
        )
        bounds = dataset.bounds
        tags = dataset.tags()
    ...
    rgb[~mask] = 0.0
    band_name = "/".join(descriptions[index - 1] for index in band_indices)
    return {
        "path": path,
        "data": rgb,
        "mask": mask,
    ...
    *,
    ...
    if has_pmw:
        ...
        if pmw is None:
            ...
            detail = " · ".join(
                value for value in (sensor, offset and f"Δt {offset} min") if value
            )
    ...
    if has_baseline:
        ...
        if baseline is None:
            panels.append((None, None, None, target_extent, "Deterministic baseline"))
        else:
    ...
    if has_map:
        ...
        _plot_valid_area_map(axes[map_index], geo, output, center, title="Valid areas")
    if has_era5_wind:
...
def _condition_plot_view(array, mask, channels):
    ...
    if array.shape[0] >= 3:
        ...
        if names:
            try:
                indices = [
                    _find_band_index(names, band) - 1 for band in ("B13", "B14", "B08")
                ]
            except ValueError:
                pass
        image = np.dstack([_normalize_channel(array[index], mask) for index in indices])
        band_name = "/".join(
            names[index] if index < len(names) else f"band {index + 1}"
            for index in indices
        )
        return image, None, band_name
    return (
        _normalize_channel(array[0], mask),
        "viridis",
        names[0] if names else "band 1",
    )
...
def _plot_era5_wind_speed_map(
    ...
    *,
    ...
    wind_speed = np.ma.masked_where(~np.asarray(valid_mask, dtype=bool), wind_speed)
    image = ax.imshow(
        wind_speed,
        extent=extent,
        origin="upper",
    ...
    ax.grid(color="white", linestyle="--", linewidth=0.6, alpha=0.65, zorder=4)
    ax.set_aspect("equal", adjustable="box")
...
def _plot_valid_area_map(
    ...
    *,
    ...
    handles = [
        ...
        Patch(
            facecolor="tab:orange", edgecolor="tab:orange", alpha=0.3, label="SAR valid"
        ),
    ]
...
def _add_valid_area(ax: Axes, mask: Any, bounds: Any, color: str) -> None:
    ...
    x = np.linspace(
        bounds.left - pixel_width / 2,
        bounds.right + pixel_width / 2,
        width + 2,
    )
    y = np.linspace(
