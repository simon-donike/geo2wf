...
class ERA5ResidualRegressor(WindFieldLightningModule):
    """Predict a deterministic physical-wind correction to the ERA5 field.

    When ``use_era5`` is false, the same U-Net predicts the absolute physical
    wind field directly and requires no ERA5 tensors in the batch.
    ...
    def __init__(
        ...
        # Raw condition plus its mask, optionally augmented by ERA5 wind and mask.
        self.model = ResidualUNet(
            in_channels=self.condition_channels + (3 if self.use_era5 else 1),
    ...
    def forward(
        ...
        era5_wind_speed: torch.Tensor | None = None,
        era5_wind_speed_mask: torch.Tensor | None = None,
    ) -> torch.Tensor:
        """Return a residual with ERA5, or an absolute wind field without it."""
        if condition.ndim != 4:
            raise ValueError(
        ...
        feature_parts = [condition * condition_mask, condition_mask]
        if self.use_era5:
            if era5_wind_speed is None or era5_wind_speed_mask is None:
                raise ValueError("ERA5 wind and mask are required when use_era5=True")
    ...
    def predict_residual_ms(self, batch: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        return self(
            ...
            batch.get("era5_wind_speed"),
            batch.get("era5_wind_speed_mask"),
        )
    ...
    def predict_physical(self, batch: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        raw_prediction = self.predict_residual_ms(batch)
        if not self.use_era5:
            return self._bound_prediction(raw_prediction)
        era5_physical = self._single_channel(
            batch["era5_wind_speed_physical"],
            "era5_wind_speed_physical",
        ).to(device=raw_prediction.device, dtype=raw_prediction.dtype)
        return self._bound_prediction(era5_physical + raw_prediction)

    def compute_training_objective(self, batch) -> LossOutput:
        """Expose the legacy scientific objective through the shared contract."""
        return LossOutput(loss=self.training_step(batch, 0))
    ...
    def predict_batch(self, batch, request: PredictionRequest) -> PredictionBatch:
        ...
        return PredictionBatch(
            ...
            central_physical=prediction,
            baseline_physical=(
                batch.get("era5_wind_speed_physical") if self.use_era5 else None
            ),
        )
    ...
    def _batch_outputs(
        ...
        if self.use_era5:
            era5 = self._single_channel(
                batch["era5_wind_speed_physical"], "era5_wind_speed_physical"
            ).to(device=residual.device, dtype=residual.dtype)
            prediction = era5 + residual
        else:
            era5 = torch.zeros_like(residual)
            prediction = residual
    ...
    def _valid_mask(
        ...
        valid = target_mask.bool() & condition_mask.bool()
        if self.use_era5:
    ...
    def _log_statistics(self, prefix: str, statistics: torch.Tensor) -> None:
        ...
        if peak_count > 0:
            ...
            if self.use_era5:
                metrics[f"{prefix}/era5_robust_peak_mae_ms"] = (
                    statistics[13] / peak_count
                )
    ...
    def _log_exceedance_statistics(self, prefix: str, statistics: torch.Tensor) -> None:
        ...
        for threshold_index, threshold in enumerate(self.exceedance_area_thresholds_ms):
            ...
            if self.use_era5:
                metrics[f"{prefix}/era5_r{label}_area_mae_fraction"] = (
    ...
    def _require_prediction_keys(self, batch: dict[str, torch.Tensor]) -> None:
        required = {"condition", "condition_mask"}
        if self.use_era5:
            required.update(
                {
                    "era5_wind_speed",
        ...
        if missing:
            raise KeyError("wind-field batches require: " + ", ".join(missing))

    def _require_supervised_batch_keys(self, batch: dict[str, torch.Tensor]) -> None:
        self._require_prediction_keys(batch)
