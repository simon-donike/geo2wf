...
from geo2wf.diffusion import *
from geo2wf.diffusion.samplers import DDIM_Sampler, DDPM_Sampler
from geo2wf.tracking.reconstruction_media import log_wandb_reconstruction
from geo2wf.metrics.wind import RADIAL_METRIC_NAMES, radial_wind_metric_statistics
...
class PixelDiffusionConditional(WindFieldLightningModule):
    ...
    def __init__(
        ...
        self.expected_data_condition_channels = int(condition_channels) - 1
        self.lr = lr
    ...
    def compute_training_objective(self, batch) -> LossOutput:
        """Expose the established denoising objective through the shared contract."""
        return LossOutput(loss=self.training_step(batch, 0))

    @torch.no_grad()
    def predict_batch(self, batch, request: PredictionRequest) -> PredictionBatch:
        previous_seed = self.validation_seed
        self.validation_seed = int(request.seed)
        try:
            _, members = self.sample_ensemble(batch, num_samples=request.ensemble_size)
        finally:
            self.validation_seed = previous_seed
        members = members.movedim(0, 1)
        prepared = self._prepare_batch_context(batch)
        baseline = prepared.get("_residual_diffusion_baseline_physical")
        offset = batch["target_norm_offset"].to(members).unsqueeze(1)
        scale = batch["target_norm_scale"].to(members).unsqueeze(1)
        members = offset + scale * members
        return PredictionBatch(
            samples_physical=members,
            central_physical=members.mean(dim=1),
