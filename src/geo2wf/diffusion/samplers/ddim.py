...
class DDIM_Sampler(nn.Module):
    ...
    def __init__(
        ...
        if num_timesteps < 1 or num_timesteps > train_timesteps:
            raise ValueError("num_timesteps must be between 1 and train_timesteps")
        if eta < 0:
        ...
        previous_lookup = torch.full((self.train_timesteps,), -2, dtype=torch.long)
        previous_lookup[timesteps] = previous_timesteps

        self.register_buffer("timesteps", timesteps, persistent=False)
        self.register_buffer("previous_timesteps", previous_timesteps, persistent=False)
        self.register_buffer(
            "_previous_timestep_lookup", previous_lookup, persistent=False
        )
        self.register_buffer("final_alpha_cumprod", torch.tensor(1.0), persistent=False)
    ...
    def step(
        ...
        *,
        ...
        direction_scale = (1 - alpha_bar_prev - std_dev.square()).clamp_min(0).sqrt()
        previous_sample = (
    ...
    def estimate_origin(
        ...
        one_minus_alpha_bar_sqrt = _extract(self.alphas_one_minus_cumprod_sqrt, t, x_t)
    ...
    def _previous_alpha_cumprod(
        ...
        return values.view(reference.shape[0], *((1,) * (reference.ndim - 1)))
