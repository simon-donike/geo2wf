...
class DDPM_Sampler(nn.Module):
    ...
    def __init__(
        ...
        posterior_variance = (
            betas * (1 - alphas_cumprod_prev) / (1 - alphas_cumprod).clamp_min(1e-20)
        )
        ...
        self.register_buffer(
            ...
            betas * alphas_cumprod_prev.sqrt() / (1 - alphas_cumprod).clamp_min(1e-20),
            persistent=False,
        )
        self.register_buffer(
    ...
    def step(
        ...
        *,
        ...
        nonzero_mask = (
            (t > 0).to(dtype=x_t.dtype).view(x_t.shape[0], *((1,) * (x_t.ndim - 1)))
        )
