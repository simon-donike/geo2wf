...
def _prepare_sampler(default_sampler, sampler, forward_process, device):
    ...
    if selected.betas.shape != forward_process.betas.shape or not torch.equal(
        selected.betas, forward_process.betas
    ):
    ...
    *,
    ...
    for timestep in iterator:
        ...
        if (
            ...
            unconditional_input = torch.cat([x_t, unconditional_condition], dim=1)
...
class DenoisingDiffusionProcess(nn.Module):
    ...
    def forward(
        ...
        sampler = _prepare_sampler(self.sampler, sampler, self.forward_process, device)
...
class DenoisingDiffusionConditionalProcess(nn.Module):
    ...
    def diffusion_terms(self, output, condition, t=None):
        ...
        alpha_bar = (
            ...
            .view(batch_size, *((1,) * (output.ndim - 1)))
            .to(device=device, dtype=output.dtype)
        )
        clean_prediction = (
            output_noisy - (1.0 - alpha_bar).clamp_min(0).sqrt() * noise_hat
    ...
    def p_loss(
        ...
        *,
        ...
        timestep_weight = self.min_snr_weight(details["snr"], min_snr_gamma)
        loss = self.weighted_loss(details["squared_error"], mask, timestep_weight)
        if return_details:
